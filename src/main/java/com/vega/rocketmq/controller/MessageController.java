...
public class MessageController {
     ...
     * 发送普通消息（同步方式）
     *
     * <p>测试方式：{@code POST http://localhost:8080/api/message/simple}
     * Body: {@code "测试消息内容"}</p>
     */
    @PostMapping("/simple")
    public String sendSimpleMessage(@RequestBody String message) {
        // TODO: 调用 simpleMessageProducer.syncSend() 发送消息
        throw new UnsupportedOperationException("TODO: 调用生产者发送普通消息");
    }

    @PostMapping("/simple/async")
    public String sendSimpleMessageAsync(@RequestBody String message) {
        // TODO: 调用 simpleMessageProducer.asyncSend() 发送异步消息
        // 提示：异步发送立即返回，结果通过回调打印到日志
        throw new UnsupportedOperationException("TODO: 调用生产者异步发送消息");
    }

    @PostMapping("/simple/oneway")
    public String sendSimpleMessageOneWay(@RequestBody String message) {
        // TODO: 调用 simpleMessageProducer.sendOneWay() 单向发送
        throw new UnsupportedOperationException("TODO: 调用生产者单向发送消息");
    }
     ...
     * 发送顺序消息
     *
     * <p>测试方式：{@code POST http://localhost:8080/api/message/orderly?bizKey=order-123}
     * Body: {@code "订单消息内容"}</p>
     *
     * <p>观察点：同一个 bizKey 的消息，消费者端是按发送顺序接收的</p>
     */
    @PostMapping("/orderly")
    public String sendOrderlyMessage(@RequestBody String message,
                                     @RequestParam String bizKey) {
        // TODO: 调用 orderlyMessageProducer.sendOrderlyMessage() 发送顺序消息
        throw new UnsupportedOperationException("TODO: 发送顺序消息");
    }
     ...
     * 发送事务消息
     *
     * <p>测试方式：{@code POST http://localhost:8080/api/message/transaction}
     * Body: {@code "事务消息内容"}</p>
     *
     * <p>观察点：</p>
     * <ul>
     ...
     * 发送延时消息
     *
     * <p>测试方式：{@code POST http://localhost:8080/api/message/delay?delayLevel=3}
     * Body: {@code "延时消息内容"}</p>
     *
     * <p>delayLevel 对照表（只能用 1-18）：</p>
     * <pre>
     * 1=1s   2=5s   3=10s   4=30s   5=1m   6=2m   7=3m   8=4m   9=5m
     * 10=6m  11=7m  12=8m  13=9m  14=10m 15=20m 16=30m 17=1h 18=2h
     * </pre>
     *
     * <p>观察点：消费者不会立即收到消息，而是在指定时间后才收到</p>
     */
    @PostMapping("/delay")
