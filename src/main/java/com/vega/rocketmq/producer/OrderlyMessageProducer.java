 ...
 * <h3>对比 RabbitMQ：</h3>
 * <p>
 * RabbitMQ 只能在单个 Queue + 单个 Consumer 下勉强保证顺序，
 * RocketMQ 的顺序保障更灵活——只要指定相同的 bizKey 就行。
 * </p>
 *
 * @author Vega.z
 */
...
public class OrderlyMessageProducer {
     ...
     *
     * <p>
     * 关键点：第三个参数 {@code hashKey} 决定了消息路由到哪个 MessageQueue。
     * 同一个 hashKey 的消息总是进入同一个队列，从而保证顺序。
     * </p>
     *
     * <p>
     * 实现思路：
    ...
    public void sendOrderlyMessage(String topic, String tag, String message, String hashKey) {
        // 使用 rocketMQTemplate.syncSendOrderly() 实现顺序消息发送
        ...
        log.info("顺序消息发送成功，hashKey={}, msgId={}, queue={}",
                hashKey, result.getMsgId(), result.getMessageQueue());
    }
}
