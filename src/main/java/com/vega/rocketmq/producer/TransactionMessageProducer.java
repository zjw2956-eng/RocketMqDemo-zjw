 ...
 *
 * <p>
 * 如果第3步因为网络问题没执行，Broker 会定期<b>回查</b>（check）生产者，
 * 询问本地事务的最终状态。这就是 {@code RocketMQLocalTransactionListener} 的职责。
 * </p>
 *
 * <pre>
 ...
 * <h3>对比 RabbitMQ：</h3>
 * <p>
 * RabbitMQ 没有原生事务消息支持，需要自己实现本地消息表 + 定时任务补偿。
 * </p>
 *
 * @author Vega.z
 */
...
public class TransactionMessageProducer {
    ...
    public void sendTransactionMessage(String topic, String tag, String message, Object arg) {
        // 使用 rocketMQTemplate.sendMessageInTransaction() 发送事务消息
        ...
        // 构造Spring Message，把业务参数放在header里
        Message<String> msg = MessageBuilder
                .withPayload(message)
                .setHeader("arg", arg) // 传给 executeLocalTransaction
                .build();
        String destination = topic + ":" + tag;
        // sendMessageInTransaction 会触发 executeLocalTransaction
        rocketMQTemplate.sendMessageInTransaction(destination, msg, arg);
        log.info("事务消息已发送（半消息状态），arg={}", arg);
    }
}
