 ...
 * <h3>关键点：</h3>
 * <p>
 * {@code checkLocalTransaction} 需要查询数据库确认本地事务的最终状态，
 * 绝对不能返回 UNKNOWN 死循环！
 * </p>
 *
 * @author Vega.z
...
@Slf4j
@Component
@RocketMQMessageListener(topic = RocketMqConstant.TOPIC_TRANSACTION, consumerGroup = RocketMqConstant.GROUP_TRANSACTION, selectorExpression = "*")
public class TransactionMessageConsumer
     ...
     *
     * @param msg 包含业务参数的消息
     * @param arg 发送时传入的额外参数（见
     *            {@link com.vega.rocketmq.producer.TransactionMessageProducer}）
    ...
    public RocketMQLocalTransactionState executeLocalTransaction(Message msg, Object arg) {
        ...
        log.info("执行本地事务，arg={}, payload={}", arg, msg.getPayload());
        // 模拟：如果 arg 字符串包含 "fail" 就失败
        String argStr = arg != null ? arg.toString() : "";
        if (argStr.contains("fail")) {
            log.warn("本地事务失败，回滚消息");
    ...
    public RocketMQLocalTransactionState checkLocalTransaction(Message msg) {
        ...
        log.info("Broker 回查事务状态，payload={}", msg.getPayload());
        // 生产环境：查数据库确认业务是否已执行
        // Demo 环境：直接返回 COMMIT
        return RocketMQLocalTransactionState.COMMIT;
    }
